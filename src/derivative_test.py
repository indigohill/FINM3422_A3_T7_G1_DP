...
class Derivative:
    ...
    def __init__(self, S0, K, T, sigma, yield_curve, dividend_yield=0.0):
        ...
        for name, val in [("S0", self.S0), ("K", self.K), ("T", self.T), ("sigma", self.sigma)]:
            if val <= 0:
    ...
    def _bump(self, **kwargs):
        ...
        params = dict(S0=self.S0, K=self.K, T=self.T, sigma=self.sigma,
                      yield_curve=self.yield_curve, dividend_yield=self.dividend_yield)
    ...
    def gamma_fd(self, h=0.01):
        ...
        """
        return (self._bump(S0=self.S0 + h) - 2 * self.price() + self._bump(S0=self.S0 - h)) / h ** 2
    ...
    def vega_fd(self, h=0.001):
        ...
        """
        return (self._bump(sigma=self.sigma + h) - self._bump(sigma=self.sigma - h)) / (2 * h) / 100
    ...
    def rho_fd(self, h=0.0001):
        ...
        """
        return (self._bump(yield_curve=ShiftedCurve(self.yield_curve, +h)) -
                self._bump(yield_curve=ShiftedCurve(self.yield_curve, -h))) / (2 * h) / 100
