...
def load_raw() -> pd.DataFrame:
    ...
    df = pd.read_excel(
        RAW_PATH, sheet_name=0, skiprows=data_start_row, header=None
    )  # header = None says that none of the rows are treated as column names.
    ...
    )  # coerce turns any unparseable values into NaT (Not a Time) rather than crashing.
    df = df.dropna(
        subset=["date"]
    )  # Remove any rows where data couldn't be parsed (blank rows in Excel).
    ...
    df = df.dropna(
        subset=maturities, how="all"
    )  # Remove any rows where every single rate column in missing.
    df = df.sort_values("date").reset_index(
        drop=True
    )  # Sort by date to ensure chronological order, then reset index.
...
def sanity_check(df: pd.DataFrame) -> None:
    ...
    assert (
        not df.empty
    ), "DataFrame is empty after cleaning."  # Ensure the DataFrame isn't empty after cleaning.
    assert df[
        "date"
    ].is_monotonic_increasing, "Dates are not in ascending order"  # Check dates are in ascending order which is a requirement for time series.
...
def load_yield_curve_data(use_cache: bool = True) -> pd.DataFrame:
    ...
    if use_cache and os.path.exists(CACHE_PATH):
        ...
        df.columns = [
            "date"
        ] + maturities  # Reapply column names as CSV doesn't preserve our float column names.
    ...
    )  # index = False prevents the pandas from writing the row numbers as an extra column.
...
def get_latest_yields() -> tuple:
    ...
    yields = {
        mat: latest[mat] for mat in maturities
    }  # Build a dictionary mapping each maturity to its zero-rate.
    return date, yields
